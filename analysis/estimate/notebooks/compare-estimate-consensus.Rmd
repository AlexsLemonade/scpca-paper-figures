...
We mostly see the same trends as observed with pseudobulk-derived ESTIMATE scores, except there is no relationship for stromal with SCPCP000017 with bulk-derived ESTIMATE scores as there was for pseudobulk.


Let's compare now the other cell types to a tumor purity proxy (sum of ESTIMATE scores):
...
ggplot(unknown_df) + 
  ...
  facet_wrap(vars(project_id), nrow = 1) +
  labs(
    x = "ESTIMATE score (bulk)", 
    y = "Fraction of non-immune/non-stromal\ncell types"
  )
```

We see the same trends as observed with pseudobulk-derived ESTIMATE scores.
