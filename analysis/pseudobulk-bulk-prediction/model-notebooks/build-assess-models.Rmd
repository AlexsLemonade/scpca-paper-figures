...
In addition, the notebook parameter `expr_threshold` determines how genes are filtered from the data before modeling: Only genes which are expressed in at least `2 * expr_threshold` in both modalities combined are retained.
...
prepare_data <- function(pseudo_file, bulk_df, id, exclude_samples, expressed_df) {
  ...
  keep_genes <- expressed_df |>
    dplyr::filter(
      project_id == id, 
      (pseudobulk_frac_expressed + bulk_frac_expressed) > (params$expr_threshold * 2)
    ) |>
...
all_models |>
  purrr::iwalk(
    \(model_list, project_id) {
      ...
      filter_str <- ifelse(params$expr_threshold < 0, "no-filter", glue::glue("{params$expr_threshold}-filter"))
