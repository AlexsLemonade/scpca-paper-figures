...
```{r}
all_models <- readr::read_rds(here::here("all_models.rds"))
...
We'll export the additive models as RDS files separately for each project.
...
all_models |>
  purrr::iwalk(
    ...
    readr::write_rds(
      model_list[["fit_additive"]], 
      file.path(output_dir, glue::glue("{project_id}_bulk-pseudobulk-model.rds"))
    )
  }
)
```
