...
* `fit_interaction`: `bulk ~ pseudobulk + (1|sample_id) + (1|sample_id:pseudobulk)`. _Only built if filtering threshold is 0._
...
model_data <- function(project_df, threshold = params$filter_threshold) {
  model_list <- list(
  ...
  if (threshold == 0) {
    model_list[["fit_interaction"]] = lmer(bulk ~ pseudobulk + (1|sample_id) + (1|sample_id:pseudobulk), data = project_df, REML = FALSE)
  }
