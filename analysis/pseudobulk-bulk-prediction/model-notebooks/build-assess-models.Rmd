...
In addition, the notebook parameter `filter_genes` determines if genes are filtered from the data before modeling:

* If this is `FALSE`, no filtering is performed
* If this is `TRUE`, genes which are not expressed at least one sample in each modalities (bulk and pseudobulk) are removed

In this notebook, the `filter_genes` parameter is `r as.logical(params$filter_genes)`.
...
```{r}
prepare_data <- function(pseudo_file, bulk_df, id, exclude_samples, expressed_df, filter = params$filter_genes) {
  
  # Find genes to keep if filtering is on
  if (filter) {
    keep_genes <- expressed_df |>
      dplyr::filter(
        project_id == id, 
        pseudobulk_frac_expressed > 0 | bulk_frac_expressed > 0
    ) |>
    dplyr::pull(ensembl_id)
  } else {
    keep_genes <- expressed_df$ensembl_id
  } 
...
* The interaction models do not usually converge - they have lots of parameters!
...
all_models |>
  purrr::iwalk(
    \(model_list, project_id) {
      ...
      filter_str <- ifelse(params$filter_genes, "filtered", "unfiltered")
      output_file <- file.path(output_dir, glue::glue("{project_id}_bulk-pseudobulk-model_{filter_str}.rds"))
      readr::write_rds(
        list(
