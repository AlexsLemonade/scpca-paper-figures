...
n_projects <- 5
stopifnot(
  "Data files not found. Check the provided model file extension." = length(data_files) == n_projects
)

ranked_residuals_list <- data_files |>
  purrr::map(
  \(f) {
    readr::read_tsv(f) |>
      # get a ranked list
      dplyr::group_by(ensembl_id) |>
      dplyr::summarize(median_res = median(residual)) |>
