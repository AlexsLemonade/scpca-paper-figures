...
We'll export both the additive model objects as RDS files and the underlying data, including Ensembl IDs and residuals, as TSV files separately for each project.
...
all_models |>
  purrr::iwalk(
    \(model_list, project_id) {
      ...
      fit <- model_list[["fit_additive"]]
      readr::write_rds(
        fit, 
        file.path(
          output_dir, 
          glue::glue("{project_id}_bulk-pseudobulk-model_{threshold_str}.rds")
        )
      )

      # Prepare and export data to tsv file
      output_data <- project_df_list[[project_id]] |>
        dplyr::mutate(residual = resid(fit)) |>
        readr::write_tsv(
          file.path(
            output_dir, 
            glue::glue("{project_id}_bulk-pseudobulk-data_{threshold_str}.tsv")
          )
        )
  }
)
