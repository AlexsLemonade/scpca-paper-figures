...
However, in both cases, the relationship appears driven by an outlying point, and/or values are generally always close to 0 so the comparisons are not very meaningful in the first palce.
...
But, there is a positive relationship for projects `SCPCP000006` and `SCPCP000009`, and the latter is actually quite close to unity (although there may not be much to interpret there).
That said, there may not be much to interpret from this panel, since references have different cell types in the first place so we expect different proportions in results.
