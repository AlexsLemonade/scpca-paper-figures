...
In addition to `quanTIseq` result TSVs stored in `../results/`, this notebook also assumes that the ScPCA sample metadata is available at `../../../s3_files/scpca-sample-metadata.tsv`.
...
#' @param use_fill Boolean for whether there should be a fill. Default is FALSE
#' @param fillvar Variable to use for fill. Only used if use_fill is TRUE
#' @param facet_rows Number of rows for facetting
#'
#' @returns ggplot object
make_barplot <- function(df, xvar, yvar, facetvar, use_fill = FALSE, fillvar = NULL, facet_rows = 1) {
...
We'll make a stacked barplot showing the proportion of "Other" cells and classified cells at sample level:
...
The percentage of classified cells in each sample ranges from close to 0 to up to around 0.5.
SCPCP000001 and SCPCP000002, which were generated by the same lab group, tend to have higher values, _potentially suggesting_ somewhat lower tumor purity in these samples if we assume that the "Other" proportion is a proxy for percentage of tumor cells.
Other projects tend to have lower lower proportions of classified cells and some appear very close to 0; there is no other overlap of generating lab.
...
Of the 10 celltypes in `quanTIseq`, how many are represented in each sample?
...
Second, in what proportion of samples is each cell type represented?
...
Most samples have at least 6/7 cell types represented.
