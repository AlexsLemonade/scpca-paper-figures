...
make_barplot <- function(df, xvar, yvar, facetvar, use_fill = FALSE, fillvar = NULL, facet_rows = 1) {
  ...
  # Note that we use this use_fill variable since an `is.null()` check won't work on a variable
  #  being tidyeval'd, and syntax to get around that can get gnarly
  if (use_fill) {
    p <- p + 
      aes(fill = {{fillvar}}) + 
...
sample_order <- celltype_counts_df |>
  dplyr::group_by(project_diagnosis) |>
  dplyr::arrange(num_celltypes) |>
  dplyr::pull(sample_id)
...
make_barplot(
  ...
  xlab("Sample arranged by number of cell types present") 
```  

Here, we see that only all samples have at least two cell types, and only one sample was inferred to contain all 10 cell types.
What's the median number of cells per sample, all pooled together?

```{r}
median(celltype_counts_df$num_celltypes)
```
