...
perm_df <- samples |>
  purrr::map(
    \(x) {
      ...
      # nonparametric
      models <- purrr::map(perms$perm, \(perm_data) wilcox.test(tpm ~ tumor_gene, alternative = "less", data = perm_data))
      ...
      # parametric
      models <- purrr::map(perms$perm, \(perm_data) t.test(tpm ~ tumor_gene, alternative = "less", data = perm_data))
      glanced <- purrr::map_df(models, broom::glance, .id = "id")
...
We'll visualize the P-value distributions; the red line is at 0.05.
