...
tpm_df <- tpm_files |>
  purrr::map(
    \(x) {
      # read matrix and convert to table
      readr::read_rds(x) |>
        as.data.frame() |>
  ...
  purrr::reduce(dplyr::left_join, by = "gene_symbol") |>
...
```
From a very quick look at a great many jitter plots, there does not seem to be a universal trend of abberantly high expression among tumor genes.
...
perm_df <- samples |>
  purrr::map(
    \(x) {
      ...
      # Create permutations by shuffling tpm values
      perms <- modelr::permute(sample_tpm, params$permutation_reps, tpm)
      
      # nonparametric
      models <- purrr::map(perms$perm, ~ wilcox.test(tpm ~ tumor_gene, data = .))
      glanced <- purrr::map_df(models, broom::glance, .id = "id")
      mod <- wilcox.test(tpm ~ tumor_gene, alternative = "less", data = sample_tpm)
      wilcox_p <- mean(glanced$statistic > broom::glance(mod)$statistic)
      
      # parametric
      models <- purrr::map(perms$perm, ~ t.test(tpm ~ tumor_gene, data = .))
      glanced <- purrr::map_df(models, broom::glance, .id = "id")
      mod <- t.test(tpm ~ tumor_gene, alternative = "less", data = sample_tpm)
      ttest_p <- mean(glanced$statistic > broom::glance(mod)$statistic)
...
ggplot(perm_df) + 
  ...
  geom_vline(xintercept = 0.05, color = "firebrick", lwd = 0.5) +
  xlim(0, 1)
```
...
Therefore, our samples do not show evidence that the `quanTIseq` tumor genes (again, these are immune-related genes identified from TCGA) have abberantly high expression compared to other signature genes.
